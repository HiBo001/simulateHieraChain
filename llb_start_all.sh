#!/bin/bash

# start_all.sh - 启动所有分片的 hierachain 节点（lldb 调试模式）

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BUILD_DIR="$PROJECT_ROOT/build/bin"
HIERACHAIN="$BUILD_DIR/hierachain"

# 颜色输出
GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  启动 Hierachain 所有分片节点${NC}"
echo -e "${GREEN}  (lldb 调试模式)${NC}"
echo -e "${GREEN}========================================${NC}"

# 检查可执行文件是否存在
if [ ! -f "$HIERACHAIN" ]; then
    echo -e "${RED}错误: 未找到 hierachain 可执行文件${NC}"
    echo -e "${RED}请先运行 make 编译项目${NC}"
    exit 1
fi

# 启用 core dump（macOS）
echo -e "${BLUE}配置 core dump 环境...${NC}"
ulimit -c unlimited
sudo sysctl kern.coredump=1 2>/dev/null
sudo launchctl limit core unlimited 2>/dev/null

# 查找所有分片目录
shard_dirs=$(find "$PROJECT_ROOT" -maxdepth 1 -type d -name "shard*" | sort)

if [ -z "$shard_dirs" ]; then
    echo -e "${RED}错误: 未找到任何 shard 目录${NC}"
    exit 1
fi

# 启动每个分片节点
for shard_dir in $shard_dirs; do
    shard_name=$(basename "$shard_dir")
    
    # 检查分片目录下是否有 shardId 文件
    if [ ! -f "$shard_dir/shardId" ]; then
        echo -e "${YELLOW}警告: $shard_name 缺少 shardId 文件，跳过${NC}"
        continue
    fi
    
    echo -e "${GREEN}启动 $shard_name ...${NC}"
    
    # 方法1：使用 lldb 运行，自动捕获崩溃并输出堆栈
    cat > "$shard_dir/lldb_commands.txt" << EOF
run
bt
quit
EOF
    
    # 切换到分片目录，使用 lldb 运行程序
    # 输出会同时保存到 node.log 和崩溃时的堆栈信息
    (
        cd "$shard_dir"
        # 使用 lldb 运行，并将输出重定向到 node.log
        lldb "$HIERACHAIN" -b -s lldb_commands.txt > "node.log" 2>&1 &
        
        # 等待一下让 lldb 启动
        sleep 0.5
    )
    
    # 记录 PID（lldb 进程）
    sleep 0.2
    pid=$(pgrep -f "lldb.*$HIERACHAIN" | head -1)
    if [ -n "$pid" ]; then
        echo $pid > "$shard_dir/node.pid"
        echo -e "${GREEN}  PID: $pid (lldb)${NC}"
    fi
done

echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}所有分片节点已启动 (lldb 调试模式)${NC}"
echo -e "${YELLOW}注意: 程序崩溃时会自动输出堆栈到 node.log${NC}"
echo -e "${GREEN}日志文件: shard*/node.log${NC}"
echo -e "${GREEN}PID 文件: shard*/node.pid${NC}"
echo -e "${GREEN}========================================${NC}"
